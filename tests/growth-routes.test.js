...
test("growth recommendation lifecycle review route is Owner-only and delegates to service", async () => {
  const calls = [];
  const authCalls = [];
  const server = createServer({
    pluginService: {
      ...
      authorizeWorkspace(input) {
        ...
        return { workspace_id: input.workspaceId, hermes_workspace_id: input.workspaceId };
      }
    },
    learningRecommendationLifecycleService: {
  ...
  const baseUrl = await listen(server);
  try {
    const memberResponse = await fetch(`${baseUrl}/api/v1/growth/recommendations/lifecycle/review`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-hermes-plugin-actor-role": "workspace",
      ...
      body: JSON.stringify({
        ...
        trajectoryId: "lgtraj_route_pending",
        status: "skipped"
      })
    });
    assert.equal(memberResponse.status, 403);
    assert.equal((await memberResponse.json()).error.code, "growth_recommendation_lifecycle_owner_required");

    const ownerResponse = await fetch(`${baseUrl}/api/v1/growth/recommendations/lifecycle/review`, {
      method: "POST",
      headers: {
        authorization: "Bearer owner-workspace-key",
        "content-type": "application/json",
        "x-hermes-plugin-actor-role": "owner",
        "x-hermes-plugin-workspace-id": "weixin_stephen"
      },
      body: JSON.stringify({
        ...
        trajectoryId: "lgtraj_route_pending",
        status: "skipped",
        reasonCode: "too_much_pressure_today",
        reviewedAt: "2026-06-16T05:00:00.000Z"
      })
    });
    assert.equal(ownerResponse.status, 200);
    const ownerBody = await ownerResponse.json();
    assert.equal(ownerBody.recommendation.status, "skipped");
    assert.equal(JSON.stringify(ownerBody).includes("owner-workspace-key"), false);
    assert.deepEqual(authCalls[0], {
      authorizationToken: "owner-workspace-key",
      workspaceId: "weixin_stephen"
    });
    assert.deepEqual(calls[0], {
      workspaceId: "weixin_fanfan",
      ...
      statusUpdatedAt: "2026-06-16T05:00:00.000Z",
...
test("growth profile feedback route is Owner-only and delegates summary-only completed-cycle selectors", async () => {
  const calls = [];
  let nextResult = null;
  const server = createServer({
    pluginService: {
      ...
      viewTargets(input) {
        if (input.actorRole === "owner") {
          return {
            ...
            current_workspace_id: input.currentWorkspaceId,
            targets: [
              { workspaceId: "weixin_stephen", label: "Owner", current: true },
              { workspaceId: "weixin_fanfan", label: "凡凡", learnerId: "fanfan" }
            ]
          };
        }
        return {
  ...
  const baseUrl = await listen(server);
  try {
    const ownerResponse = await fetch(`${baseUrl}/api/v1/growth/profile-feedback?workspaceId=growth:weixin_fanfan&learnerId=fanfan&programId=program_science&domainPackId=uk_hk_curriculum_foundation&domain=science&subject=science&horizon=daily_plan&availableMinutes=15&planDraftId=lgplan_route_1&taskCardId=ltask_route_1&evaluationId=leval_route_1&profileDeltaId=lgpdelta_route_1&evidenceId=lgevd_route_1&correctionId=lgcorr_route_1&sourceId=leval_route_1&targetNodeIds=kg_science_fair_test,kg_science_variables&autoSelectCompletedCycle=true&autoSelectLatestCompletedCycle=true&limit=9`, {
      headers: {
        "x-hermes-plugin-actor-role": "owner",
        "x-hermes-plugin-workspace-id": "weixin_stephen"
      }
    ...
    assert.equal(ownerResponse.status, 200);
    const ownerBody = await ownerResponse.json();
    assert.equal(ownerBody.schemaVersion, "growth.learningProfileFeedbackEvidence.v1");
    assert.equal(ownerBody.privacyClass, "summary_only");
    assert.equal(ownerBody.summary.readyForNextPlan, true);
    assert.equal(ownerBody.loopState.nextAction.action, "draft_daily_plan");
    assert.deepEqual(calls[0], {
      workspaceId: "weixin_fanfan",
      learnerId: "fanfan",
      displayName: "凡凡",
      ...
      planDraftId: "lgplan_route_1",
      taskCardId: "ltask_route_1",
      evaluationId: "leval_route_1",
      profileDeltaId: "lgpdelta_route_1",
      evidenceId: "lgevd_route_1",
      correctionId: "lgcorr_route_1",
      sourceId: "leval_route_1",
      targetNodeIds: ["kg_science_fair_test", "kg_science_variables"],
      limit: "9",
      domainPackId: "uk_hk_curriculum_foundation",
      domain: "science",
    ...
    const missingSelectorResponse = await fetch(`${baseUrl}/api/v1/growth/profile-feedback?workspaceId=weixin_fanfan&learnerId=fanfan`, {
      headers: {
        "x-hermes-plugin-actor-role": "owner",
        "x-hermes-plugin-workspace-id": "weixin_stephen"
      }
    });
    assert.equal(missingSelectorResponse.status, 200);
    const missingSelectorBody = await missingSelectorResponse.json();
    assert.equal(missingSelectorBody.ok, false);
    assert.equal(missingSelectorBody.privacyClass, "summary_only");
    assert.equal(missingSelectorBody.status, "missing");
    assert.deepEqual(missingSelectorBody.summary.missingRequired, ["cycle_selector_present"]);

    const deniedRole = await fetch(`${baseUrl}/api/v1/growth/profile-feedback?workspaceId=weixin_stephen`, {
      headers: {
        "x-hermes-plugin-actor-role": "workspace",
        "x-hermes-plugin-workspace-id": "weixin_stephen"
      }
    ...
    assert.equal((await deniedRole.json()).error.code, "growth_profile_feedback_owner_required");

    const deniedTarget = await fetch(`${baseUrl}/api/v1/growth/profile-feedback?workspaceId=weixin_unknown`, {
      headers: {
        "x-hermes-plugin-actor-role": "owner",
        "x-hermes-plugin-workspace-id": "weixin_stephen"
      }
  ...
  try {
    ...
    assert.deepEqual(calls[3], {
      ...
      input: {
        ...
        evidence: {
          ...
          releasePackageReviewUiEvidence: undefined,
    ...
    assert.deepEqual(calls[19], {
      ...
      input: {
        ...
        proposalReviewUiEvidenceFile: undefined,
        releasePackageReviewUiEvidenceFile: undefined,
        automationDigestUiEvidenceFile: undefined,
        automationActionHandoffUiEvidenceFile: undefined,
        schedulerExecutionUiEvidenceFile: undefined,
...
test("growth automation central visual evidence route validates inline visible-target summary", async () => {
  const calls = [];
  const server = createServer({
    pluginService: {
      getManifest: () => ({}),
  ...
  const baseUrl = await listen(server);
  try {
    const response = await fetch(`${baseUrl}/api/v1/growth/automation/central-visual-evidence`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-hermes-plugin-actor-role": "owner",
      ...
      body: JSON.stringify({
        ...
        centralVisualEvidence: {
          ...
          status: "pass",
          screenshotPresent: true,
          screenshotFile: "growth-embedded.png",
          assertions: [{ key: "embedded_plugin_shell", ok: true }]
        }
      })
    });
    assert.equal(response.status, 200);
    const body = await response.json();
    assert.equal(body.schemaVersion, "growth.learningAutomationCentralVisualEvidence.v1");
    assert.equal(body.visualEvidence.screenshotPresent, true);
    assert.equal(body.centralBoundary.homeAiOwnsVisualHarness, true);
    assert.equal(body.centralBoundary.growthRunsNoAppium, true);
    assert.equal(body.centralBoundary.growthReadsOnlyCentralHarnessArtifacts, true);
    assert.equal(body.writefulSchedulingAllowed, false);
    ...
    assert.equal(calls[0].centralVisualEvidenceFile, undefined);

    const denied = await fetch(`${baseUrl}/api/v1/growth/automation/central-visual-evidence`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-hermes-plugin-actor-role": "workspace",
      ...
      body: JSON.stringify({ workspaceId: "weixin_fanfan", centralVisualEvidence: { ok: true } })
    });
    assert.equal(denied.status, 403);
    assert.equal((await denied.json()).error.code, "growth_target_not_visible");
  } finally {
...
test("growth automation release artifact template route returns visible-target manifest template", async () => {
      ...
      template(input) {
        ...
        return {
          ...
          releaseArtifactTemplate: {
            ...
            artifactSlotCount: 1,
            artifactTaskIds: ["central_visual"],
  ...
  const baseUrl = await listen(server);
  try {
    const response = await fetch(`${baseUrl}/api/v1/growth/automation/release-artifact-template?workspaceId=growth:weixin_fanfan&learnerId=fanfan&collectionRunId=lgacrn_route_1&activationGates=writeful_execution&requiredApprovalKey=writefulExecutionApproval&limit=4&runtimeEnablementRecordLimit=6`, {
      headers: {
        "x-hermes-plugin-actor-role": "owner",
        "x-hermes-plugin-workspace-id": "weixin_stephen"
      }
    ...
    assert.equal(response.status, 200);
    const body = await response.json();
    assert.equal(body.schemaVersion, "growth.learningAutomationReleaseEvidenceArtifactTemplate.v1");
    assert.equal(body.releaseArtifactTemplate.artifactSlotCount, 1);
    assert.equal(body.releaseArtifactTemplate.artifactManifestTemplate.centralVisualEvidenceFile, "");
    assert.deepEqual(calls[0], {
      workspaceId: "weixin_fanfan",
      learnerId: "fanfan",
      displayName: "凡凡",
    ...
    const denied = await fetch(`${baseUrl}/api/v1/growth/automation/release-artifact-template?workspaceId=weixin_fanfan`, {
      headers: {
        "x-hermes-plugin-actor-role": "workspace",
        "x-hermes-plugin-workspace-id": "weixin_stephen"
      }
...
test("growth automation release workbench action route is Owner-write and visible-target scoped", async () => {
  ...
  try {
    ...
    assert.deepEqual(calls[0], {
      ...
      activationGate: undefined,
	      activationGates: [],
      releaseEvidenceBundle: undefined,
      releaseEvidenceBundleAudit: undefined,
      releaseReadiness: undefined,
      releaseCollectionRun: undefined,
      releaseDecision: undefined,
      releaseEvidenceBundleFile: undefined,
      ...
      ownerAuthorizedWrite: true
	    ...
	    const collectionResponse = await fetch(`${baseUrl}/api/v1/growth/automation/release-workbench/actions`, {
	      ...
	      body: JSON.stringify({
	        ...
	        release_package_review_ui_evidence_file: "/tmp/release-package-review-ui.json",
	        scheduler_run_ui_evidence_file: "/tmp/scheduler-run-ui.json",
	        write_collection_run: true,
	        write_release_evidence_records: true,
	        requested_by: "weixin_owner",
	    ...
	    assert.equal((await collectionResponse.json()).endpointKey, "release_evidence_collection");
	    assert.deepEqual(calls[1], {
	      workspaceId: "weixin_fanfan",
	      learnerId: "fanfan",
	      displayName: "凡凡",
...
test("growth planner readiness route is Owner-only and delegates through orchestrator", async () => {
  const calls = [];
  const server = createServer({
    pluginService: {
      getManifest: () => ({}),
    ...
    learningPlanOrchestratorService: {
      async smokePlannerReadiness(input) {
        calls.push(input);
        return {
          ok: true,
          ...
          context: {
            ...
            learnerId: input.learnerId,
            subject: input.subject,
            candidateNodeCount: input.targetNodeIds.length,
            privacyClass: "summary_only"
          },
          draftSummary: {
  ...
  const baseUrl = await listen(server);
  try {
    const response = await fetch(`${baseUrl}/api/v1/growth/automation/planner-readiness?targetWorkspaceId=weixin_fanfan&learnerId=fanfan&programId=program_science&domainPackId=uk_hk_curriculum_foundation&domain=science&subject=science&horizon=daily_plan&availableMinutes=15&targetNodeIds=kg_science_fair_test`, {
      headers: {
        "x-hermes-plugin-actor-role": "owner",
        "x-hermes-plugin-workspace-id": "weixin_stephen"
      }
    ...
    const body = await response.json();
    assert.equal(body.ok, true);
    assert.equal(body.context.workspaceId, "weixin_fanfan");
    assert.deepEqual(calls[0], {
      workspaceId: "weixin_fanfan",
      learnerId: "fanfan",
      displayName: "凡凡",
    ...
    const memberDenied = await fetch(`${baseUrl}/api/v1/growth/automation/planner-readiness?targetWorkspaceId=weixin_stephen`, {
      headers: {
        "x-hermes-plugin-actor-role": "workspace",
        "x-hermes-plugin-workspace-id": "weixin_stephen"
      }
    ...
    assert.equal(memberDenied.status, 403);
    assert.equal((await memberDenied.json()).error.code, "growth_planner_readiness_owner_required");
  } finally {
    await close(server);
  }
});
...
test("growth learning loop state is Owner-only and limited to visible targets", async () => {
  ...
  const baseUrl = await listen(server);
  try {
    const ownerResponse = await fetch(`${baseUrl}/api/v1/growth/learning-loop/state?workspaceId=growth:weixin_fanfan&learnerId=fanfan&programId=program_science&domainPackId=uk_hk_curriculum_foundation&domain=science&subject=science&subjectId=science&capabilityClusterId=science.fair_test&horizon=daily_plan&availableMinutes=15&targetNodeIds=kg_science_fair_test,kg_science_variables&assessmentCoverageNodeIds=kg_science_fair_test,kg_science_variables&planDraftId=lgplan_route_1&itemId=plan_item_route_1&taskCardId=ltask_route_1&limit=5`, {
      headers: {
        "x-hermes-plugin-actor-role": "owner",
        "x-hermes-plugin-workspace-id": "weixin_stephen"
      }
...
test("automation cycle closure prepare route is Owner scoped and delegates to service", async () => {
  const calls = [];
  const server = createServer({
    pluginService: {
      getManifest: () => ({}),
    ...
    learningAutomationCycleClosureService: {
      async prepareReviewPacket(input) {
        ...
        return {
          ...
          stages: [
            { name: "profile_feedback", ok: true },
            { name: "proposal_create", ok: true },
            { name: "proposal_review", ok: true },
            { name: "digest_create", ok: true }
          ],
          writesPerformed: true,
          publishPerformed: false,
          schedulerStarted: false
  ...
  const baseUrl = await listen(server);
  try {
    const prepared = await fetch(`${baseUrl}/api/v1/growth/automation/cycle-closures/prepare`, {
      method: "POST",
      headers: {
        authorization: "Bearer owner-token",
        "content-type": "application/json",
    ...
    assert.equal(preparedBody.summary.proposalId, "lgauto_route_1");
    assert.equal(preparedBody.publishPerformed, false);
    assert.equal(preparedBody.schedulerStarted, false);
    assert.deepEqual(calls[0], {
      workspaceId: "weixin_fanfan",
      learnerId: "fanfan",
      displayName: "凡凡",
      ...
      sourceTargetNodeIds: undefined,
      cycleId: undefined,
      sourcePlanDraftId: undefined,
      sourceTaskCardId: undefined,
      sourceEvaluationId: undefined,
      profileDeltaId: undefined,
      evidenceId: undefined,
      correctionId: undefined,
      sourceId: undefined,
      ...
      createDigest: true,
      reviewDigest: false,
      digestReviewStatus: undefined,
      digestReviewNote: undefined,
      selectedCandidateIds: undefined,
      createHandoff: false,
      deliverHandoff: undefined,
    ...
    const deniedRole = await fetch(`${baseUrl}/api/v1/growth/automation/cycle-closures/prepare`, {
      method: "POST",
      headers: {
        authorization: "Bearer owner-token",
        "content-type": "application/json",
    ...
    assert.equal(deniedRole.status, 403);
    assert.equal((await deniedRole.json()).error.code, "growth_automation_cycle_closure_owner_required");

    const deniedTarget = await fetch(`${baseUrl}/api/v1/growth/automation/cycle-closures/prepare`, {
      method: "POST",
      headers: {
        authorization: "Bearer owner-token",
        "content-type": "application/json",
