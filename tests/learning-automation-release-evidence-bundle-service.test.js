...
test("release evidence bundle service builds summary-only bundle from no-write smoke tasks", () => {
  ...
  const result = service.buildBundle({
    ...
    tasks: ["planner_readiness", "daily_loop_preview", "learning_loop_state", "cycle_history", "owner_audit", "profile_feedback", "learner_cycle", "stage_assessment", "proposal"],
    requestedBy: "owner"
  });
  ...
  assert.equal(result.bundle.summary.taskCount, 9);
  ...
  assert.ok(calls[8].args[0].endsWith("scripts/smoke-growth-automation-proposal.js"));
