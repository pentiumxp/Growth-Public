...
test("release package script can write a summary-only package record to Growth SQLite", () => {
  withTempDb(({ dir, dbPath }) => {
    ...
    assert.equal(output.record.package.releaseDashboardSummary.readinessEvidenceMissingCount, 35);
    ...
    try {
      ...
      assert.equal(JSON.parse(row.release_dashboard_summary_json).readinessEvidenceMissingCount, 35);
    } finally {
      db.close();
    }
  });
