...
test("release workbench composes release services into Owner action templates without writes", () => {
  ...
  assert.equal(stateActions.every((action) => action.externalActionRequired === false), true);
  ...
  assert.equal(digestAction.route.method, "POST");
  assert.equal(digestAction.route.path, "/api/v1/growth/automation/release-workbench/actions");
  assert.equal(digestAction.route.body.endpoint_key, "automation_digest");
  assert.equal(digestAction.route.body.action_key, "reviewed_automation_digest");
  assert.equal(digestAction.route.body.digest_id, "");
  assert.equal(digestAction.followupRoute.method, "GET");
  assert.equal(digestAction.followupRoute.path, "/api/v1/growth/automation/digests");
  assert.equal(digestAction.followupRoute.query.status, "reviewed");
  assert.equal(digestAction.followupRoute.query.workspace_id, "fanfan");
  assert.deepEqual(digestAction.requiredSelectorKeys, ["digest_id"]);
  ...
  assert.equal(policyAction.route.body.endpoint_key, "automation_failure_policy");
  assert.equal(policyAction.route.body.policy_id, "");
  assert.equal(policyAction.route.body.status, "active");
  assert.equal(policyAction.followupRoute.path, "/api/v1/growth/automation/failure-policies/readiness");
  ...
  assert.equal(handoffAction.route.body.endpoint_key, "automation_action_handoff");
  assert.equal(handoffAction.route.body.handoff_id, "");
  assert.equal(handoffAction.route.body.digest_id, "");
  assert.equal(handoffAction.followupRoute.path, "/api/v1/growth/automation/action-handoffs");
  assert.equal(handoffAction.followupRoute.query.deliveryStatus, "delivered");
  assert.deepEqual(handoffAction.requiredSelectorKeys, ["handoff_id"]);
  assert.deepEqual(handoffAction.fallbackSelectorKeys, ["digest_id"]);
  ...
  assert.equal(workerTargetAction.route.body.endpoint_key, "automation_scheduler_worker_target");
  assert.equal(workerTargetAction.route.body.target_id, "");
  assert.equal(workerTargetAction.route.body.status, "enabled");
  assert.equal(workerTargetAction.followupRoute.path, "/api/v1/growth/automation/scheduler/worker-targets");
  ...
  assert.equal(result.releaseWorkbench.recordRoutes.some((route) => route.key === "automation_digest"), true);
  assert.equal(result.releaseWorkbench.recordRoutes.some((route) => route.key === "automation_failure_policy"), true);
  assert.equal(result.releaseWorkbench.recordRoutes.some((route) => route.key === "automation_action_handoff"), true);
  assert.equal(result.releaseWorkbench.recordRoutes.some((route) => route.key === "automation_scheduler_worker_target"), true);
