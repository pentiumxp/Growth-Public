...
function validateInput(input = {}) {
  if (!input.workspaceId) return { ok: false, error: "release_dashboard_smoke_workspace_required" };
  return { ok: true };
}
...
async function main() {
  ...
  const result = runOperation(services.learningAutomationReleaseDashboardService, input);
  process.stdout.write(formatResult(Object.assign({ operation: "dashboard" }, result), pretty));
  process.exitCode = 0;
}
...
if (require.main === module) {
  main().catch((error) => {
    process.stdout.write(formatResult({
      ok: false,
      error: "release_dashboard_smoke_failed",
