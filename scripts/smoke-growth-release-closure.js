...
function validateInput(input = {}) {
  if (!input.workspaceId) return { ok: false, error: "release_closure_smoke_workspace_required" };
  return { ok: true };
}
...
async function main() {
  ...
  const result = runOperation(services.learningAutomationReleaseClosureService, input);
...
if (require.main === module) {
  main().catch((error) => {
    process.stdout.write(formatResult({
      ok: false,
      error: "release_closure_smoke_failed",
