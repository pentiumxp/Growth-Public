...
| Operating-loop release evidence | `node --test tests/learning-automation-release-evidence-bundle-service.test.js tests/growth-release-evidence-bundle-script.test.js tests/growth-operating-loop-smoke-script.test.js tests/growth-architecture-boundary.test.js`; the release bundle default task `operating_loop_history` must collect only no-write `list-runs` evidence into `productionOperatingLoopHistorySmokeEvidence` and must not execute the operating loop. |
