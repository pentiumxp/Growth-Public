...
| Learning automation release activation service | `src/services/learning-automation-release-activation-service.js` | No-write activation preflight service for Owner runtime-config decisions after release closure. It composes `learning-automation-release-closure-service.summarize`, validates the closure DTO boundary, maps selected activation gates (`writeful_execution`, `background_scheduler`, `background_worker`) to approval keys/config keys/env names, and projects `growth.learningAutomationReleaseActivation.v1` with `preflightPassed`, `readyForOwnerRuntimeConfigDecision`, current config state, missing approvals, required actions, and next action. It backs visible-target scoped `GET /api/v1/growth/automation/release-activation` and `npm run smoke:release-activation`. It writes no tables, applies no config change, keeps `configChangeApplied=false`, `writefulSchedulingAllowed=false`, and `runtimeConfigChange=false`, and must not call Gateway, publish plans, generate cards, evaluate submissions, execute scheduler actions, run scheduler ticks, deliver notifications, activate stage assessments, mutate learner state, or inspect SQLite tables directly. |
...
| Release activation preflight gate | `node --test tests/learning-automation-release-activation-service.test.js tests/growth-release-activation-smoke-script.test.js tests/growth-routes.test.js tests/growth-architecture-boundary.test.js` and `npm run smoke:release-activation`; the CLI defaults to no-write preflight, reads only through the normal service graph, combines release-closure readback with selected runtime config gate state, supports `writeful_execution`, `background_scheduler`, and `background_worker` activation gates, returns a summary-only activation DTO, keeps `configChangeApplied=false`, `writefulSchedulingAllowed=false`, and `runtimeConfigChange=false`, and must not import repositories, write tables, call Gateway, publish, generate, evaluate, execute scheduler actions, run scheduler ticks, deliver notifications, activate stage assessments, flip runtime config, or mutate learner state. |
