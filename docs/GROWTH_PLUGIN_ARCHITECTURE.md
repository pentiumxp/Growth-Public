...
- Release-evidence-safe: release readiness is an evidence aggregate, not an
  ...
  still keep `writefulSchedulingAllowed=false`.
...
| Release package gate | `node --test tests/learning-automation-release-package-service.test.js tests/growth-release-package-script.test.js tests/learning-automation-release-evidence-bundle-service.test.js tests/learning-automation-release-evidence-bundle-audit-service.test.js tests/learning-automation-release-readiness-service.test.js tests/learning-automation-release-collection-run-service.test.js tests/learning-automation-release-controls-service.test.js tests/growth-architecture-boundary.test.js` and `npm run smoke:release-package`; the CLI composes bundle, bundle audit, readiness, collection-run, and controls into one summary-only package. It defaults to no-write, can persist only a collection-run audit row with `--write-collection-run --allow-write`, and must not call Gateway, publish, generate, evaluate, execute scheduler actions, run scheduler ticks, deliver notifications, activate stage assessments, flip runtime config, grant scheduler permission, inspect SQLite directly, or mutate learner state. |
