...
- Release-evidence-safe: release readiness is an evidence aggregate, not an
  ...
  summarizes product, platform, visual, action-handoff smoke,
...
8. Release-readiness evidence gate:
   ...
   - aggregate only summary-only evidence for product UI, audit UI, proposal /
     ...
     handoffs, production action-handoff smoke, production
     scheduler-execution smoke, production scheduler-run smoke, reviewed
     worker-target smoke, reviewed enabled worker targets, production planner
     readiness, controlled daily-loop write smoke, production dry-run, central
     visual evidence, platform Action Inbox/Web Push evidence, and explicit
     release approval;
...
| Automation release-readiness gate | `node --test tests/learning-automation-release-readiness-repository.test.js tests/learning-automation-release-readiness-service.test.js tests/growth-release-readiness-smoke-script.test.js tests/growth-routes.test.js tests/growth-architecture-boundary.test.js` and `npm run smoke:release-readiness`; the service requires production action handoff smoke evidence from `npm run smoke:action-handoff`, production scheduler execution smoke evidence from `npm run smoke:scheduler-execution`, production scheduler run smoke evidence from `npm run smoke:scheduler-run`, production scheduler worker target smoke evidence from `npm run smoke:scheduler-worker-target`, production scheduler worker smoke evidence from `npm run smoke:scheduler-worker`, and production daily-loop preview smoke evidence from `npm run smoke:daily-loop-preview` before `readyForReleaseReview`, keeps the CLI no-write by default, and must not call Gateway, daily-loop services, publication, evaluation, scheduler execution, scheduler ticks, notification delivery, stage activation, learner-state mutation, or direct repositories from the CLI. |
