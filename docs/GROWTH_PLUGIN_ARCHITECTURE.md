...
| Automation release-readiness gate | `node --test tests/learning-automation-release-readiness-repository.test.js tests/learning-automation-release-readiness-service.test.js tests/growth-release-readiness-smoke-script.test.js tests/growth-routes.test.js tests/growth-architecture-boundary.test.js` and `npm run smoke:release-readiness`; the service requires automation digest/action handoff/scheduler execution/scheduler run/worker-target UI evidence, production action handoff smoke evidence from `npm run smoke:action-handoff`, production scheduler execution smoke evidence from `npm run smoke:scheduler-execution`, production scheduler run smoke evidence from `npm run smoke:scheduler-run`, production scheduler worker target smoke evidence from `npm run smoke:scheduler-worker-target`, production scheduler worker smoke evidence from `npm run smoke:scheduler-worker`, production planner readiness smoke evidence from `npm run smoke:planner-readiness`, production daily-loop preview smoke evidence from `npm run smoke:daily-loop-preview`, production scheduler dry-run smoke evidence from `npm run smoke:scheduler-dry-run`, and an internal no-write scheduler dry-run safety check before `readyForReleaseReview`; the smoke CLI accepts summary-only `growth.learningAutomationReleaseEvidenceBundle.v1` input through file or inline JSON, keeps no-write by default, and must not call Gateway, daily-loop services, publication, evaluation, scheduler execution, scheduler ticks, notification delivery, stage activation, learner-state mutation, or direct repositories from the CLI. |
