...
| Operational smoke scripts | `scripts/smoke-growth-planner-readiness.js`, `scripts/smoke-growth-daily-loop-preview.js`, `scripts/smoke-growth-daily-loop.js`, `scripts/smoke-growth-owner-audit.js`, `scripts/smoke-growth-release-readiness.js`, `scripts/smoke-growth-scheduler-dry-run.js`, `scripts/smoke-growth-automation-digest.js`, `scripts/smoke-growth-automation-failure-policy.js`, `scripts/smoke-growth-automation-action-handoff.js` | CLI-only evidence collectors that instantiate the normal service graph and delegate to service boundaries. Planner readiness is no-write. Daily-loop preview is no-write and calls only `learningDailyLoopService.preview`. Controlled daily-loop smoke defaults to preview and requires explicit `--allow-write` for draft/publish evidence through `learningDailyLoopService`; publish additionally requires `--plan-draft-id`. Owner audit defaults to read-only cycle audit, completeness, and correction readback; its correction operation requires explicit `--allow-write` and delegates only to `learningOwnerCorrectionService.recordCorrection` before refreshing audit DTOs. Scheduler dry-run is no-write and calls only `learningAutomationSchedulerService.dryRun`. Digest smoke defaults to read-only `learningAutomationDigestService.listDigests`; `get` is read-only, while `create` and `review` require explicit `--allow-write` and call only `learningAutomationDigestService`. Failure policy smoke defaults to read-only `learningAutomationFailurePolicyService.evaluateReadiness`; `list` is read-only, while `create` and `review` require explicit `--allow-write` and call only `learningAutomationFailurePolicyService`. Action handoff smoke defaults to read-only `learningAutomationActionHandoffService.listHandoffs`; `create` and `deliver` require explicit `--allow-write` and call only `learningAutomationActionHandoffService`. Release readiness is no-write by default and writes only an explicit summary-only snapshot with `--write-snapshot`. Scripts must not import SQLite repositories directly, call Gateway directly, call plan-publisher/card-generation/evaluation directly, execute scheduler actions, run scheduler ticks, or activate stage assessments. Only the action handoff smoke may deliver a bounded Growth automation event, and only through `learningAutomationActionHandoffService.deliverHandoff`. |
