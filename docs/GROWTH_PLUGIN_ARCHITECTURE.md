...
| Release review smoke | `scripts/smoke-growth-release-review.js` | CLI glue for `npm run smoke:release-review`. It delegates through the normal service graph to `learning-automation-release-review-service.review`, returns `growth.learningAutomationReleaseReview.v1`, and reads current readiness plus latest collection run, latest decision, and approval bag through existing service boundaries. It is always no-write and must not run smoke tasks, import repositories, call Gateway, publish, generate, evaluate, execute scheduler actions, run scheduler ticks, deliver notifications, activate stage assessments, mutate learner state, or flip runtime config. |
...
| Learning automation release review service | `src/services/learning-automation-release-review-service.js` | No-write summary readback service for Owner release controls. It composes release-readiness evaluation, latest collection-run list, latest decision list, and release approval bag through existing service boundaries; projects `growth.learningAutomationReleaseReview.v1` with status, next action, advisory flags, latest artifacts, and approval summary; backs visible-target scoped `GET /api/v1/growth/automation/release-review` and `npm run smoke:release-review`; and keeps `advisoryOnly=true`, `runtimeConfigChange=false`, and `writefulSchedulingAllowed=false`. It must not write repositories or tables, run smoke tasks, spawn commands, call Gateway, publish plans, generate cards, evaluate submissions, execute scheduler actions, run scheduler ticks, deliver notifications, activate stage assessments, mutate learner state, flip runtime config, or inspect SQLite tables directly. |
| Learning automation release authorization service | `src/services/learning-automation-release-authorization-service.js` | No-write final authorization service for writeful execution. It reads the release-review DTO through `learning-automation-release-review-service`, then authorizes only when the review schema is `growth.learningAutomationReleaseReview.v1`, privacy is `summary_only`, review status is `approved`, the latest collection run is `ready_for_release_review`, the latest decision is `approved`, and `writefulExecutionApproval` is active. It backs visible-target scoped `GET /api/v1/growth/automation/release-authorization` and `npm run smoke:release-authorization`; scheduler execution consumes it after local handoff/digest/policy/dry-run gates. It writes no tables, flips no config, keeps `writefulSchedulingAllowed=false` and `runtimeConfigChange=false`, and must not call Gateway, publish plans, generate cards, evaluate submissions, execute scheduler actions, run scheduler ticks, deliver notifications, activate stage assessments, mutate learner state, or inspect SQLite tables directly. |
| Learning automation release closure service | `src/services/learning-automation-release-closure-service.js` | No-write release-control summary service for Owner/release tooling. It composes `learning-automation-release-review-service.review` and `learning-automation-release-authorization-service.authorize`, then projects `growth.learningAutomationReleaseClosure.v1` with review status, execution gate status, sanitized latest collection/decision summaries, `backendEvidenceComplete`, `readyForOwnerReleaseActivation`, missing check/evidence/approval keys, required actions, and next action. It backs visible-target scoped `GET /api/v1/growth/automation/release-closure` and `npm run smoke:release-closure`. It writes no tables, flips no config, keeps `writefulSchedulingAllowed=false` and `runtimeConfigChange=false`, and must not call Gateway, publish plans, generate cards, evaluate submissions, execute scheduler actions, run scheduler ticks, deliver notifications, activate stage assessments, mutate learner state, or inspect SQLite tables directly. |
