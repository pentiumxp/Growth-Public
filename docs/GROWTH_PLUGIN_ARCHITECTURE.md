...
- Release-evidence-safe: release readiness is an evidence aggregate, not an
  ...
  scheduler-execution smoke, scheduler-run smoke, daily-loop write-smoke,
  dry-run, worker-target smoke, reviewed-target, config, and approval evidence,
  but it must not call Gateway,
