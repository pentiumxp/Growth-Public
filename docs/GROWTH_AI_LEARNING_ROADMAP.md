...
| Automation release readiness | `learning-automation-release-readiness-service`, `automation-release-readiness.js`, `learning-automation-release-evidence-bundle-audit-service`, `learning-automation-release-collection-run-service`, `automation-release-collection-runs.js`, `learning-automation-release-package-service`, `learning-automation-release-decision-service`, `automation-release-decisions.js` | Evaluate advisory release-readiness checks, derive bounded missing/blocked/evidence/next-action remediation fields, consume platform action evidence from `learning-automation-platform-action-evidence-service`, consume central visual evidence from `learning-automation-central-visual-evidence-service`, consume release evidence bundle self-audit evidence from `learning-automation-release-evidence-bundle-audit-service`, persist optional summary-only snapshots, persist sanitized release collection-run records, compose summary-only release package artifacts from bundle/audit/readiness/collection-run/controls readback, and persist a separate summary-only Owner release decision after a collection run exists. `approved` requires a ready collection run. It is not a runtime config switch and must not enable execution, scheduling, Gateway calls, evaluation, notification delivery, visual-tool execution, stage activation, or learner-state writes. |
