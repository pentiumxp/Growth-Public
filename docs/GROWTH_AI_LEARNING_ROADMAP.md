...
| L9: Release-readiness evidence | Growth can summarize product, platform, visual, dry-run, reviewed-target, config, approval, cycle-history smoke, Owner audit smoke, profile-feedback smoke, controlled daily-loop write-smoke, learner-cycle audit smoke, platform action receipt evidence, central visual artifact evidence, smoke-bundle evidence, smoke-bundle self-audit, bounded source-bundle/readiness evidence readback, persisted collection-run evidence, release package artifacts, persisted package audit records, and package-record readback in release review/authorization/closure/controls without enabling execution or scheduling. | Release-readiness repository/service/route tests, release collection-run repository/service/route/script tests, release package repository/service/route/script tests, platform action evidence service/script tests, central visual evidence service/script tests, release evidence bundle service/script tests, release evidence bundle audit service/script tests, `tests/learning-automation-release-readiness-repository.test.js`, `tests/growth-release-readiness-smoke-script.test.js`, `tests/growth-release-collection-run-smoke-script.test.js`, `tests/growth-release-package-script.test.js`, `tests/learning-automation-release-package-repository.test.js`, `tests/growth-platform-action-evidence-smoke-script.test.js`, `tests/growth-central-visual-evidence-smoke-script.test.js`, `tests/growth-release-evidence-bundle-script.test.js`, `tests/growth-release-evidence-bundle-audit-smoke-script.test.js`, release approval bag collection through the bundle builder, default `cycle_history` bundle task coverage, default `owner_audit` bundle task coverage, default `profile_feedback` bundle task coverage, default `platform_action` bundle task coverage, default `central_visual` bundle task coverage, default audit-only `learner_cycle` bundle task coverage, explicit write-gated `daily_loop_write` bundle task coverage, bundle self-audit coverage for schema/summary/task/evidence/privacy/path checks, collection-run coverage for sanitized bundle/audit/readiness persistence, release package coverage for bundle/audit/readiness/collection-run/controls/dashboard composition and write-gated collection-run plus package-record persistence including dashboard summary, release-review/authorization/closure/controls package readback and latest-package dashboard summary coverage, visible-target/Owner-only package API coverage, privacy rejection including private path/token-like values in readback catalogs, disabled-config evidence, missing-evidence states, `releaseReview` remediation plan coverage for missing/blocked/evidence keys, persisted `evidenceReadback` coverage for present/missing counts and per-check evidence ids/status/source, package-record status, and next action, architecture no-Gateway/no-publication/no-evaluation/no-scheduler/no-Action-Inbox-internal/no-visual-tooling/no-stage-activation guard, docs-locality checks, and broad local validation pass. |
...
| G9: Release-readiness evidence | Can Growth state exactly which release prerequisites are present or missing and persist sanitized collection and decision records? | A release-readiness boundary and `npm run smoke:release-readiness` return bounded `pass`/`missing`/`blocked`/`not_applicable` checks plus `releaseReview` remediation fields and summary-only `evidenceReadback` for product UI, audit UI, stage-checkpoint smoke, proposal/digest/action/execution/run UI, active policy, delivered handoffs, reviewed targets, disabled/enabled config gates, platform Action Inbox/Web Push receipt evidence from `npm run smoke:platform-action-evidence`, central visual evidence from `npm run smoke:central-visual-evidence` over a Home AI visual harness artifact, production proposal smoke, production learning-loop state smoke, production cycle-history smoke, production Owner audit smoke, production profile-feedback smoke, production controlled daily-loop write smoke, production learner-cycle audit smoke, production dry-run evidence, release evidence bundle self-audit from `npm run smoke:release-evidence-bundle-audit`, collection-run evidence from `npm run smoke:release-collection-run`, release package evidence from `npm run smoke:release-package`, Owner decision evidence from `npm run smoke:release-decision`, and explicit release approval. It persists the readback catalog only as summary audit data and must not publish, schedule, call Gateway, evaluate, activate stage assessments, deliver notifications, run visual tooling, call cycle-history, Owner audit, learner-cycle services, or Home AI Action Inbox/Web Push internals, flip runtime config, or mutate learner state. |
...
- no-write profile-feedback evidence service and `npm run smoke:profile-feedback`
  for completed-cycle audit/evidence/profile-delta/Profile V2/recommendation/
  next-state readback;
...
- service-level Fanfan science vertical harness through planner draft,
  ...
  profile delta, profile-feedback evidence, and next loop-state readback;
...
| Automation release readiness | `learning-automation-release-readiness-service`, `automation-release-readiness.js`, `learning-automation-release-evidence-bundle-audit-service`, `learning-automation-release-collection-run-service`, `automation-release-collection-runs.js`, `learning-automation-release-package-service`, `automation-release-packages.js`, `learning-automation-release-decision-service`, `automation-release-decisions.js` | Evaluate advisory release-readiness checks, derive bounded missing/blocked/evidence/next-action remediation fields, project and optionally persist summary-only `evidenceReadback` catalogs in `evidence_readback_json`, consume platform action evidence from `learning-automation-platform-action-evidence-service`, consume central visual evidence from `learning-automation-central-visual-evidence-service`, consume release evidence bundle self-audit evidence from `learning-automation-release-evidence-bundle-audit-service`, persist optional summary-only snapshots, persist sanitized release collection-run records, compose summary-only release package artifacts from bundle/audit/readiness/collection-run/controls/dashboard readback, persist summarized release package audit records including dashboard summaries, and persist a separate summary-only Owner release decision after a collection run exists. `approved` requires a ready collection run. It is not a runtime config switch and must not enable execution, scheduling, Gateway calls, evaluation, notification delivery, visual-tool execution, stage activation, or learner-state writes. |
