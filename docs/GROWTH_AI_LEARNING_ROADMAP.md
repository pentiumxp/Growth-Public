...
| G9: Release-readiness evidence | Can Growth state exactly which release prerequisites are present or missing and persist a sanitized collection run? | A release-readiness boundary and `npm run smoke:release-readiness` return bounded `pass`/`missing`/`blocked`/`not_applicable` checks plus `releaseReview` remediation fields for product UI, audit UI, stage-checkpoint smoke, proposal/digest/action/execution/run UI, active policy, delivered handoffs, reviewed targets, disabled/enabled config gates, platform Action Inbox/Web Push receipt evidence from `npm run smoke:platform-action-evidence`, central visual evidence from `npm run smoke:central-visual-evidence` over a Home AI visual harness artifact, production proposal smoke, production learning-loop state smoke, production cycle-history smoke, production Owner audit smoke, production profile-feedback smoke, production controlled daily-loop write smoke, production learner-cycle audit smoke, production dry-run evidence, release evidence bundle self-audit from `npm run smoke:release-evidence-bundle-audit`, collection-run evidence from `npm run smoke:release-collection-run`, and explicit release approval. It must not publish, schedule, call Gateway, evaluate, activate stage assessments, deliver notifications, run visual tooling, call cycle-history, Owner audit, learner-cycle services, or Home AI Action Inbox/Web Push internals, or mutate learner state. |
...
| Automation release readiness | `learning-automation-release-readiness-service`, `automation-release-readiness.js`, `learning-automation-release-evidence-bundle-audit-service`, `learning-automation-release-collection-run-service`, `automation-release-collection-runs.js` | Evaluate advisory release-readiness checks, derive bounded missing/blocked/evidence/next-action remediation fields, consume platform action evidence from `learning-automation-platform-action-evidence-service`, consume central visual evidence from `learning-automation-central-visual-evidence-service`, consume release evidence bundle self-audit evidence from `learning-automation-release-evidence-bundle-audit-service`, persist optional summary-only snapshots, and persist sanitized release collection-run records for release review only. It is not a runtime config switch and must not enable execution, scheduling, Gateway calls, evaluation, notification delivery, visual-tool execution, stage activation, or learner-state writes. |
