...
| L9: Release-readiness evidence | Growth can summarize product, platform, visual, dry-run, reviewed-target, config, approval, cycle-history smoke, Owner audit smoke, profile-feedback smoke, controlled daily-loop write-smoke, learner-cycle audit smoke, platform action receipt evidence, and smoke-bundle evidence without enabling execution or scheduling. | Release-readiness repository/service/route tests, platform action evidence service/script tests, release evidence bundle service/script tests, `tests/growth-release-readiness-smoke-script.test.js`, `tests/growth-platform-action-evidence-smoke-script.test.js`, `tests/growth-release-evidence-bundle-script.test.js`, release approval bag collection through the bundle builder, default `cycle_history` bundle task coverage, default `owner_audit` bundle task coverage, default `profile_feedback` bundle task coverage, default `platform_action` bundle task coverage, default audit-only `learner_cycle` bundle task coverage, explicit write-gated `daily_loop_write` bundle task coverage, privacy rejection, disabled-config evidence, missing-evidence states, `releaseReview` remediation plan coverage for missing/blocked/evidence keys and next action, architecture no-Gateway/no-publication/no-evaluation/no-scheduler/no-Action-Inbox-internal/no-stage-activation guard, docs-locality checks, and broad local validation pass. |
...
| Automation release readiness | `learning-automation-release-readiness-service`, `automation-release-readiness.js` | Evaluate advisory release-readiness checks, derive bounded missing/blocked/evidence/next-action remediation fields, consume platform action evidence from `learning-automation-platform-action-evidence-service`, and persist optional summary-only snapshots for release review only. It is not a runtime config switch and must not enable execution, scheduling, Gateway calls, evaluation, notification delivery, stage activation, or learner-state writes. |
