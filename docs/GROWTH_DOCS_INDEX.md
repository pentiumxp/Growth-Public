...
| `docs/GROWTH_AI_LEARNING_NEXT_STAGE_PLAN.md` | Durable next-stage plan for the supervised AI learning system: end-to-end closed-loop mechanism, current capability summary, current execution decision, next architecture optimization target, product-visible Owner loop path, backend release-readiness gate path, release-readiness smoke/snapshot CLI, readiness DTO and snapshot contract, Fanfan science daily playbook, readiness semantics, harness matrix, and definition of done. |
