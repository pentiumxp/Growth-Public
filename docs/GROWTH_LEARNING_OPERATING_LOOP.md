...
Owner `生成` tab can now list bounded proposals, record `accepted`/`skipped`,
and explicitly publish an accepted proposal through the proposal publish route.
It is not yet enough to claim a complete learning operating system because
proposal creation from a selected complete cycle, digest/action/execution UI,
multi-card weekly plans, production execution of the planner readiness smoke,
...
- the supervised automation proposal boundary is implemented locally as the
  ...
  Owner `生成` tab now exposes the list/review/accepted-publish part of this
  layer, while proposal creation from selected complete cycles remains a later
  UI slice.
