...
| Evaluation layer | Gateway evaluation, bounded DTO, visible failure/retry, evidence derivation. | Implemented with Gateway/fallback boundary plus service-owned subject-aware rubric validation. Evaluation input carries the card rubric policy, including subject-specific daily practice rubrics when available; model output may return `rubricResults`, invalid rubric dimensions fail validation, and the evidence ledger stores bounded rubric summaries in `summary_json`. Audit/Profile V2 readback now projects those rubric summaries without exposing raw learner/model content. | Render richer Owner UI over rubric evidence and continue expanding subject-specific rubric catalogs. |
...
- additional subject-specific rubric catalogs beyond the current V1 English,
  science, mathematics, history, geography, computer-science, and generic
  fallback set.
...
6. add richer subject-specific rubric catalogs and Owner audit rendering after
   ...
   computer-science, and generic fallback daily rubric policies are visually
   validated.
