...
cycle, list bounded proposals, record `accepted`/`skipped`, and explicitly
publish an accepted proposal through the proposal publish route.
It is not yet enough to claim a complete learning operating system because
`expired`/`superseded` UI, digest/action/execution UI, multi-card weekly plans,
production execution of the planner readiness smoke, and central visual/release
...
- the supervised automation proposal boundary is implemented locally as the
  ...
  accepted-publish part of this layer. `expired`/`superseded` decision UI and
