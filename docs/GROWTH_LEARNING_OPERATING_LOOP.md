...
8. Owner or UI drilldown can read `GET /api/v1/growth/learning-cycles/audit`
   for the bounded cycle timeline and
...
| Automation action handoff layer | Summary-only Owner action metadata after reviewed digest and active failure policy. | `learning-automation-action-handoff-service`, `learning_growth_automation_action_handoffs`, visible-target/Owner scoped `/api/v1/growth/automation/action-handoffs` routes, and `growth.automation.action_required` event mapping are implemented. Delivery records `delivered` or `delivery_failed` without publishing, proposal execution, Gateway, scheduler, or learner-state mutation. | Render action/delivery state in Owner automation UI and collect platform Action Inbox/Web Push evidence before writeful scheduling. |
...
- release-readiness production evidence can now be passed as a versioned
  ...
  default-disabled smoke CLIs, includes read-only proposal smoke by default,
  and emits only summary evidence. This remains
