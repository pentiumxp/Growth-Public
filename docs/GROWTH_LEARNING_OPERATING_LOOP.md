...
automation proposal layer as a dry-run Owner-review boundary, and the embedded
Owner `生成` tab can now create a bounded proposal from the selected historical
cycle, list bounded proposals, record `accepted`/`skipped`/`expired`/
`superseded`, show blocked-action feedback, explicitly publish an accepted
proposal through the proposal publish route, and refresh downstream
digest/action/scheduler/release readbacks after proposal actions.
It is not yet enough to claim a complete learning operating system because
production execution of the planner readiness smoke, production visual/UI
evidence, platform receipt evidence, release evidence, reviewed worker-target
evidence, multi-card weekly plans, and production writeful scheduling remain
later slices.
...
| Evidence ledger | Unified summary-only evidence records across evaluations, reflections, learner signals, stage assessments, and Owner-reviewed corrections. | `learning-evidence-ledger-service` and `learning_growth_evidence_ledger` are implemented for daily evaluation, formal assessment, reflection, experience-signal, and Owner-reviewed correction evidence. `learning-evidence-audit-service` and `GET /api/v1/growth/evidence/audit` provide bounded readback over persisted evidence rows for visible targets, including rubric policy ids, rubric dimension ids, weak/stable rubric dimension ids, and rubric evidence types from summary-only `summary_json`. | Add broader Owner audit UI projection over the implemented backend DTOs. |
| Profile layer | Mastery, confidence, stability, recency, misconceptions, habits, pressure signals, interests, and subject-specific descriptors. | `learning-profile-v2-service` projects ledger evidence plus optional legacy profile context, applies Owner-reviewed correction evidence as a bounded auditable state adjustment, exposes evidence-freshness metadata, projects rubric policy/dimension/evidence-type coverage into capability states, and Owner generation context now exposes a bounded Profile V2 projection. | Add richer Owner audit explanations in the embedded UI. |
...
| Authoring layer | Generate validated teaching, practice, repair, stretch, integration, project, or assessment cards. | Daily English, daily science, subject-scoped daily practice, stage assessment generation, and planner-backed single-item publication are implemented. Subject-scoped compact recipes still pass through target provisioning, graph planning, graph evidence requirements, Gateway authoring, validation, summary-only rubric policy, and transactional publishing. V1 subject practice now has stable rubric specializations for mathematics, history, geography, and computer science, plus generic fallback for unknown subjects. Formal `stage_assessment` cards now resolve `rubric:stage_assessment_v1:<subject>` independently of daily recipe defaults. | Expand the subject rubric catalog beyond the current V1 set while keeping broader objective selection on validated planner drafts. |
...
| Evaluation layer | Gateway evaluation, bounded DTO, visible failure/retry, evidence derivation. | Implemented with Gateway/fallback boundary plus service-owned subject-aware rubric validation. Evaluation input carries the card rubric policy, including subject-specific daily practice and formal stage-assessment rubrics when available; model output may return `rubricResults`, invalid rubric dimensions fail validation, SQLite evaluation records preserve bounded rubric readback, and the evidence ledger stores bounded rubric summaries in `summary_json`. Audit/Profile V2 readback now projects those rubric summaries without exposing raw learner/model content. | Render richer Owner UI over rubric evidence and continue expanding subject-specific rubric catalogs. |
| Stage layer | Formal assessment eligibility, activation, high-weight evidence, cooldown. | Implemented as a service boundary. Stage activation preserves target domain-pack/domain/subject scope, resolves formal checkpoint rubric policy, generates through the normal card-generation service, writes one high-weight formal evaluation, and closes the cycle into cooldown after the one formal reflection. | Use profile confidence and ledger freshness to suggest assessment cycles. |
| Audit layer | Owner-visible reason, evidence basis, plan publication link, score-proportional reward settlement, profile delta, model-boundary state, failure state, Owner-reviewed correction trail, historical-cycle selection, and required audit completeness. | Owner generation context shows profile, recommendation, planner readiness, planner context preview, Profile V2, bounded evidence audit items, and `ownerAudit` readback over plan-audit, persisted profile-delta, and correction DTOs. Evidence-audit readback is implemented through `learning-evidence-audit-service` and `GET /api/v1/growth/evidence/audit`; reward settlement readback is implemented through `learning-reward-audit-service` and injected into `learning-loop-state-service` as summary-only recommendation evidence; plan-audit readback is implemented through `learning-plan-audit-service`; profile-delta persistence/readback are implemented through `learning_growth_profile_delta_audits` and `GET /api/v1/growth/profile-delta-audits`; Owner-reviewed corrections are implemented through `learning-owner-correction-service`, `POST /api/v1/growth/profile-corrections`, and `GET /api/v1/growth/profile-corrections`; cycle-level readback is implemented through `learning-cycle-audit-service` and `GET /api/v1/growth/learning-cycles/audit`, including summary-only rubric policy ids, rubric dimension ids, weak/stable dimension ids, evidence types, and timeline rubric counts; selectable cycle history is implemented through `learning-cycle-history-service`, `GET /api/v1/growth/learning-cycles/history`, and `npm run smoke:cycle-history`; embedded older-cycle controls now read that DTO and use selected service-provided selectors to refresh `learning-cycles/audit` and `learning-cycles/completeness`; required audit evidence readback is implemented through `learning-audit-completeness-service` and `GET /api/v1/growth/learning-cycles/completeness`. | Collect production visual/release evidence for the embedded Owner audit surface before treating it as product-complete. |
| Automation proposal layer | Owner-reviewed proposal creation, decision, and explicit accepted-proposal publish execution before any scheduling or automatic publication. | The target boundary is `learning-automation-proposal-service`, `learning_growth_automation_proposals`, `GET`/`POST /api/v1/growth/automation/proposals`, `POST /api/v1/growth/automation/proposals/:proposalId/decision`, and `POST /api/v1/growth/automation/proposals/:proposalId/publish`: previous cycle completeness and target provisioning are required before a new plan draft can become a proposal, Owner decisions record `accepted`, `skipped`, `expired`, or `superseded`, and accepted publish execution delegates to the plan publisher while recording bounded execution metadata. The embedded Owner `生成` tab has selected-cycle create/list/review/accepted-publish controls, terminal decision controls, visible blocked-action feedback, and downstream digest/action/scheduler/release readback refresh after proposal actions. Backend Owner review evidence summarizes `proposed`, `accepted`, `skipped`, `expired`, `superseded`, owner-decision, proposal execution-status counts, and downstream digest/action-handoff/scheduler execution/scheduler run/worker-target/failure-policy stage counts, while keeping the accepted-proposal gate limited to `accepted`; `npm run smoke:owner-review-evidence` now mirrors these counters as bounded top-level `automationOwnerReviewEvidence*` operator readback. | Collect production visual/UI evidence, production dry-run evidence, platform receipt evidence, and release evidence before making proposals a primary production Owner workflow or enabling any scheduler. |
| Automation cycle closure layer | One Owner action turns a completed cycle into a supervised review packet. | `learning-automation-cycle-closure-service`, Owner-only `POST /api/v1/growth/automation/cycle-closures/prepare`, `npm run smoke:cycle-closure`, and the embedded Owner `闭环复核包` panel are implemented. The service auto-selects the latest completed cycle when no explicit selector is supplied, delegates proposal creation/review to the proposal service, delegates pending digest creation to the digest service, and can optionally delegate digest review/action handoff/delivery to their owning services. It writes no table of its own, returns summary-only `growth.learningAutomationCycleClosure.v1`, keeps `publishPerformed=false` and `schedulerStarted=false`, and never calls Gateway directly, Home AI old Growth server logic, card generation/evaluation, scheduler execution/ticks, or repositories directly. | Collect production UI evidence and complete at least one real learner cycle before treating this as the normal Owner workflow. |
| Automation review advancement layer | One Owner action advances a review packet into reviewed digest, active failure policy, and handoff readiness. | `learning-automation-review-advancement-service`, Owner-only `POST /api/v1/growth/automation/review-advancements/advance`, `npm run smoke:review-advancement`, and the embedded Owner `复核链推进` panel are implemented. The service composes cycle closure, digest review, failure-policy readiness/create/review, action handoff creation/delivery, and optional default-disabled scheduler execution evidence. It writes no table of its own, returns summary-only `growth.learningAutomationReviewAdvancement.v1`, defaults to no handoff delivery and no scheduler execution, keeps `publishPerformed=false` and `schedulerStarted=false` by default, and never calls Gateway directly, Home AI old Growth server logic, card generation/evaluation, scheduler ticks, worker timers, runtime/deploy switches, or repositories directly. | Collect production UI evidence and use the route on a real completed learner cycle before treating it as the normal Owner workflow. |
...
| Generalization layer | Workspace, learner, graph pack, subject, and recipe become parameters. | Fanfan sample, daily English, daily science, and subject-scoped ordinary daily recipes are operational; Fanfan science is covered by a service-level vertical harness; backend context exposes `graphOptions`; the embedded Owner UI renders domain-pack/subject selection and target provisioning controls. A non-sample science vertical harness now proves unprovisioned blocking, explicit provision enablement, wrong-subject blocking, and target-workspace scoped plan/card/evidence/profile/audit rows. V1 rubric policies are now subject-aware for English, science, mathematics, history, geography, computer science, and generic fallback daily practice. | Collect production visual evidence for the generalized controls and add richer subject-specific rubric catalogs beyond the current V1 set. |
...
- richer Owner audit rendering over the implemented subject rubric readbacks;
- additional subject-specific rubric catalogs beyond the current V1 English,
  science, mathematics, history, geography, computer-science, generic fallback,
  and formal stage-assessment set.
...
- the supervised automation proposal boundary is implemented locally as the
  ...
  metadata, and keeps writeful scheduling out of this layer. The embedded
  Owner `生成` tab now exposes the selected-cycle create/list/review/
  accepted-publish part of this layer, including terminal `expired` and
  `superseded` decisions, visible blocked-action feedback, and downstream
  automation readback refresh. Production visual/UI and release evidence remain
  later slices.
...
5. render richer profile-delta audit from persisted public DTOs after learner
   completion;
6. add richer subject-specific rubric catalogs and Owner audit rendering after
   the current V1 English, science, mathematics, history, geography,
   computer-science, generic fallback daily, and formal stage-assessment rubric
   policies are visually validated.
