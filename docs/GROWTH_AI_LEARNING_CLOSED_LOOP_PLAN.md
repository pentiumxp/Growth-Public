...
| Audit readback | Backend read routes expose plan, evidence, profile-delta, correction, cycle audit, selectable cycle history, profile-feedback, and completeness DTOs; the Owner `生成` tab now renders current-cycle and selected historical-cycle audit/completeness plus a readable `画像反馈` panel using service-provided selectors. | Production-complete release still requires central visual/release evidence for the audit surface, and later digest/action review UI. |
| Supervised automation | Proposal creation, Owner decision, and accepted-proposal explicit publication are implemented as a non-scheduling backend layer. The Owner `生成` tab now has a selected-cycle proposal panel that creates bounded proposals from service-provided historical-cycle selectors, lists existing proposals, records `accepted`/`skipped`/`expired`/`superseded`, shows blocked-action feedback, explicitly publishes accepted proposals, and refreshes downstream digest/action/scheduler/release readbacks after proposal actions. | Production dry-run evidence, platform receipt evidence, central visual/UI evidence, release evidence, reviewed worker-target evidence, and explicit release activation are still required before any scheduler can become production writeful. |
...
5. Complete supervised proposal UI release closure by collecting central
   visual/UI evidence, production proposal/dry-run evidence, platform receipt
   evidence, and release evidence for selected-cycle create/review/publish plus
   downstream digest/action/scheduler readbacks.
...
## Cycle Closure Review Packet

The cycle-closure packet is the Owner-facing bridge from a completed learner
cycle into the supervised automation review stack. It is implemented as a
service-first orchestrator, not as a new persistence table.

Boundary:

| Boundary | Contract |
| --- | --- |
| Service | `learning-automation-cycle-closure-service` composes `learning-profile-feedback-evidence-service`, `learning-automation-proposal-service`, `learning-automation-digest-service`, and optional action-handoff service calls. It owns orchestration only. |
| Route | Owner-only `POST /api/v1/growth/automation/cycle-closures/prepare`, workspace-bearer authorized and Growth visible-target scoped. |
| CLI harness | `npm run smoke:cycle-closure -- --allow-write` delegates only to `learningAutomationCycleClosureService.prepareReviewPacket()` and mirrors bounded `automationCycleClosure*` readback fields. `npm run smoke:home-ai-proxy -- --operation cycle-closure --allow-write` is the production same-origin proxy path for the same route and remains write-gated. |
| UI | The embedded Owner `生成` tab exposes `闭环复核包`, which prepares a review packet from the selected completed cycle or the latest service-selected completed cycle. |

Default behavior:

1. If explicit source selectors are absent, the service asks profile-feedback
   to select the latest completed cycle through summary-only cycle-history
   discovery.
2. It creates a supervised automation proposal from that completed cycle.
3. It records the default Owner proposal decision as `accepted` so the proposal
   becomes ready for later explicit publication.
4. It creates a pending automation digest over the same bounded scope.
5. It returns summary-only stage readback. It does not publish cards, start a
   scheduler, mutate learner state beyond the owned proposal/digest writes, or
   call Home AI old Growth server logic.

Optional inputs can ask the same service to review the digest, create an action
handoff, or deliver the handoff. Those steps still delegate to the owning
digest/action-handoff services and fail closed when their prerequisites are not
met, such as missing active failure policy or unreviewed digest.

The only model-entered step remains proposal creation's existing planner draft
boundary. The cycle-closure service itself must not call Gateway, model
vendors, card authoring, evaluation, scheduler execution, scheduler ticks,
...
| S3: Closed audit UI | Owner can inspect plan basis, evidence, profile delta, corrections, next recommendation, and persisted Owner review decisions after completion. | Audit UI tests, privacy tests, route tests, owner-audit-review repository/service/smoke tests, and visual evidence pass. |
...
| S5: Multi-workspace/domain-pack rollout | Any visible and provisioned learner/domain pack can run the same loop. | Cross-workspace allow/deny, provisioned target, target-switch UI, no-data-mixing tests, `npm run smoke:target-provisioning`, default release-bundle `target_provisioning`, `productionTargetProvisioningSmokeEvidence`, and release-readiness key `production_target_provisioning_smoke_evidence` pass. |
| S6: Supervised automation proposal | Growth can create, Owner-review, and explicitly publish an accepted next-learning proposal from a completed auditable cycle without starting a scheduler. | Proposal repository/service/route tests prove summary-only persistence, idempotent save, Owner decision status, accepted-only publish execution, bounded execution metadata, audit-completeness gating, target provisioning, privacy rejection, no direct Gateway/card-generation calls, and Owner-only write policy. The cycle-closure review-packet service/route/UI/CLI can auto-select the latest completed cycle, create an accepted proposal, and create a pending digest without publishing or scheduling. The embedded Owner selected-cycle create/review/publish panel is implemented and covered by frontend adapter plus architecture guards; terminal `accepted`/`skipped`/`expired`/`superseded` decisions, visible blocked-action feedback, and downstream digest/action/scheduler/release readback refresh are covered locally. Production visual/UI and release evidence remain open. |
...
| Owner audit review closure | `learning-owner-audit-review-service`, `learning_growth_owner_audit_reviews`, `GET /api/v1/growth/owner-audit/reviews`, Owner-only `POST /api/v1/growth/owner-audit/reviews`, and `npm run smoke:owner-audit-review` persist the fact that Owner reviewed one completed-cycle profile-feedback packet. The service delegates to `learning-profile-feedback-evidence-service.evaluate()` first, stores only summary fields such as readiness, counts, recommendation, next action, selector ids, decision, status, reviewer, and bounded note, and never writes learner evidence or profile state. The embedded Owner generation page exposes this as `完成周期审核`: it lists existing review rows, uses the selected history-cycle selectors to record `accepted`, `needs_follow_up`, `correction_recorded`, or `blocked`, keeps `correction_recorded` disabled without an existing correction id, and refreshes the loop/reference/readback state after a write. The browser must not compute profile feedback, fabricate cycles, create correction evidence, call Gateway, generate/evaluate cards, schedule, or mutate learner state. `correction_recorded` must reference an existing correction id; the correction itself remains `owner_reviewed_correction` evidence owned by `learning-owner-correction-service`. Harness coverage requires repository/service/route/smoke/frontend/architecture tests, explicit `--allow-write` for record, Owner-only HTTP write, visible-target read, privacy fail-closed behavior, and no Gateway, generation, evaluation, scheduler, notification, stage activation, or learner-state mutation. |
| Owner review signal readback | `learning-owner-review-signal-service` is the read-only bridge from persisted Owner audit-review rows into the next planning loop. It projects `growth.learningOwnerReviewSignal.v1` with latest decision/status, review ids/selectors, counts, and a bounded planner strategy bias. `accepted` and `correction_recorded` can strengthen the next plan input, `needs_follow_up` can bias toward low-pressure repair or Owner follow-up, and `blocked` remains visible as a blocker. The signal excludes Owner notes and raw learner/model content, is advisory for ordinary daily planning, and must not write evidence, mutate Profile V2, call Gateway, generate cards, evaluate submissions, schedule, notify, or activate stage assessments. |
...
| Owner learning-loop state readback | `learning-loop-state-service`, `learning-reward-audit-service`, Owner-only `GET /api/v1/growth/learning-loop/state`, `tests/learning-loop-state-service.test.js`, `tests/learning-reward-audit-service.test.js`, `tests/growth-learning-loop-state-smoke-script.test.js`, the post-cycle state assertion in `tests/learning-card-ai-loop-harness.test.js`, the learner-cycle-full-to-loop-state smoke chain in `tests/growth-learner-cycle-smoke-script.test.js`, `npm run smoke:learning-loop-state`, summary-only `growth.learningLoopState.v1`, nested `growth.learningLoopState.recommendationEvidence.v1`, and architecture guards against Gateway, direct repository, publication, generation, evaluation, scheduler, notification, handoff, stage activation, or learner-state mutation. The recommendation evidence trace must explain the next recommendation from bounded evidence ids, source card/evaluation ids, reward settlement ids/coin totals, plan drafts, profile-delta audits, Owner corrections, Profile V2 node summaries, and trajectory lifecycle rows without exposing raw learner/model content, idempotency keys, ledger-entry JSON, or raw settlement payloads. Scalar completed-cycle selectors such as `taskCardId` and `evaluationId` must be normalized before reward-audit readback so CLI/HTTP callers cannot lose reward evidence because a selector arrived as a single string. |
...
| Profile feedback evidence | `learning-profile-feedback-evidence-service`, Owner-only `GET /api/v1/growth/profile-feedback`, the embedded Owner `画像反馈` panel, `scripts/smoke-growth-profile-feedback.js`, `tests/learning-profile-feedback-evidence-service.test.js`, `tests/growth-profile-feedback-smoke-script.test.js`, the post-cycle profile-feedback assertion in `tests/learning-card-ai-loop-harness.test.js`, and the `profile_feedback` object type in the Growth Reference Contract prove summary-only completed-cycle audit/evidence/profile-delta/Profile V2/recommendation/next-state readback. The evidence projection carries bounded reward readback from the next loop state, including settlement count and total learning coins, plus non-required Owner review signal summaries when they exist. Missing Owner review does not block low-pressure daily next planning; it remains visible as review status. The Owner UI must render missing or blocked completed-cycle evidence as visible `待补齐` / `无完成周期` state instead of a silent no-op. The release-bundle `profile_feedback` task plus reference-contract projection preserve these fields in summary form. The default boundary still requires a completed-cycle selector and fails closed when one is absent. Only explicit `autoSelectCompletedCycle` or `autoSelectLatestCompletedCycle` input may turn read-only cycle-history discovery into a selected completed-cycle selector; the selection is summary-only, prefers the most recent completed candidate only when the broader flag is present, and must never fabricate learner history. Architecture coverage requires no Gateway, direct repository, publication, generation, evaluation, scheduler, notification, stage activation, or learner-state mutation. |
| Automation cycle closure | `learning-automation-cycle-closure-service`, Owner-only `POST /api/v1/growth/automation/cycle-closures/prepare`, `npm run smoke:cycle-closure`, `npm run smoke:home-ai-proxy -- --operation cycle-closure`, `tests/learning-automation-cycle-closure-service.test.js`, `tests/growth-automation-cycle-closure-smoke-script.test.js`, route tests, frontend adapter tests, and architecture guards prove the completed-cycle-to-review-packet bridge. The service can auto-select the latest completed cycle through profile-feedback, create a supervised proposal, record the default accepted proposal decision, create a pending digest, and optionally delegate digest review/action handoff/delivery. It returns summary-only `growth.learningAutomationCycleClosure.v1`, keeps `publishPerformed=false` and `schedulerStarted=false`, rejects privacy-risk keys, writes only through the owning proposal/digest/handoff services, and must not call Gateway directly, publish cards, generate/evaluate cards, execute scheduler actions, run scheduler ticks, activate stage assessments, inspect repositories/tables directly, notify except through the action-handoff service, mutate learner profile/evidence state, bypass the Home AI proxy boundary, or use Home AI old Growth server internals. |
...
| Automation closed-loop action plan | `learning-automation-closed-loop-action-plan-service`, Owner-only `GET /api/v1/growth/automation/closed-loop/action-plan`, `npm run smoke:closed-loop-action-plan`, `tests/learning-automation-closed-loop-action-plan-service.test.js`, `tests/growth-automation-closed-loop-action-plan-smoke-script.test.js`, route tests, and architecture guards prove the no-write next-action readback over operating-loop, profile-feedback, digest, failure-policy, and action-handoff services. It returns summary-only `growth.learningAutomationClosedLoopActionPlan.v1`, one bounded next-action template, `writePerformed=false`, `writesPerformed=false`, `publishPerformed=false`, and `schedulerStarted=false`; it must not call Gateway, publish, generate/evaluate cards, execute scheduler actions, run scheduler ticks, deliver notifications, activate stage assessments, mutate runtime config, deploy, inspect repositories/tables directly, or mutate learner state. |
...
cycle-closure review-packet boundary is implemented locally through
`learning-automation-cycle-closure-service`,
