...
| Owner audit review closure | `learning-owner-audit-review-service`, `learning_growth_owner_audit_reviews`, `GET /api/v1/growth/owner-audit/reviews`, Owner-only `POST /api/v1/growth/owner-audit/reviews`, and `npm run smoke:owner-audit-review` persist the fact that Owner reviewed one completed-cycle profile-feedback packet. The service delegates to `learning-profile-feedback-evidence-service.evaluate()` first, stores only summary fields such as readiness, counts, recommendation, next action, selector ids, decision, status, reviewer, and bounded note, and never writes learner evidence or profile state. `correction_recorded` must reference an existing correction id; the correction itself remains `owner_reviewed_correction` evidence owned by `learning-owner-correction-service`. Harness coverage requires repository/service/route/smoke/architecture tests, explicit `--allow-write` for record, Owner-only HTTP write, visible-target read, privacy fail-closed behavior, and no Gateway, generation, evaluation, scheduler, notification, stage activation, or learner-state mutation. |
