...
6. Owner explicitly publishes one selected daily item, or uses the formal
   assessment controls for a checkpoint.
...
| Audit readback | Backend read routes expose plan, evidence, profile-delta, correction, cycle audit, and completeness DTOs. | Owner UI must render those DTOs as an explanation surface and correction entry point without exposing raw private content. |
...
| S1: Learning-cycle audit aggregate | Owner can read one card/evaluation/plan cycle as a single service-owned audit DTO and can check whether required audit evidence is present before UI closure or later automation. | Implemented locally through `learning-cycle-audit-service`, `learning-audit-completeness-service`, visible-target route tests, plan publish-attempt audit tests, privacy-stripping service tests, and architecture guard coverage. |
...
| S4: Stage-checkpoint UI | Owner can activate formal checkpoints separately from daily practice. | Stage readiness/cooldown tests and no-direct-formal-publish tests pass. |
...
| S9: Release-readiness evidence | Growth can summarize product, platform, visual, production controlled daily-loop write smoke, production dry-run, reviewed-target, config, and explicit release evidence before any production scheduling decision. | Release-readiness service/repository/route harnesses prove summary-only snapshots, missing/blocked/all-pass checks, privacy rejection, visible-target scope, Owner-only snapshot writes, no Gateway, no publication, no evaluation, no scheduler execution/tick, no notification delivery, no stage activation, and `writefulSchedulingAllowed=false`. |
...
| Vertical loop | Fanfan science daily path from plan draft to card publish, learner evidence, evaluation, ledger, Profile V2, profile delta, and recommendation. |
...
| Owner learning-loop state readback | `learning-loop-state-service`, Owner-only `GET /api/v1/growth/learning-loop/state`, `tests/learning-loop-state-service.test.js`, `tests/growth-learning-loop-state-smoke-script.test.js`, `npm run smoke:learning-loop-state`, summary-only `growth.learningLoopState.v1`, and architecture guards against Gateway, direct repository, publication, generation, evaluation, scheduler, notification, handoff, stage activation, or learner-state mutation. |
| Supervised automation proposal | previous-cycle source id required; completeness gate before planner draft; target provisioning before planner draft; summary-only proposal persistence; duplicate/idempotent save; Owner decision status, terminal-decision idempotency, conflicting-decision rejection, accepted-only publish execution, successful execution idempotency, failed/blocked execution visibility, legacy decision/execution-column migration; Owner-only write routes; visible-target read route; no direct Gateway, direct card-generation, stage-assessment activation, or scheduler calls. |
...
| Automation release readiness | advisory readiness evaluation, summary-only snapshot persistence, external evidence keys, `growth.learningAutomationReleaseEvidenceBundle.v1` bundle builder evidence, active dependency checks, disabled-config checks, explicit release approval checks, Owner-only snapshot writes, visible-target list/read, privacy-risk rejection, and no Gateway/publication/evaluation/scheduler execution/tick/notification/stage activation/learner-state mutation from either readiness or bundle-builder boundaries. |
...
| UI boundary | progress state, visible errors, mobile scroll, dark mode contrast, no hidden lower controls, no silent generate action. |
...
dry-run boundary. The automation digest backend is also implemented locally:
...
UI, production planner readiness smoke, proposal review UI, scheduler/digest
UI, platform action UI or Action Inbox/Web Push evidence, and central visual
evidence still need to be completed before production rollout.
