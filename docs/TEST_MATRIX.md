...
| --- | --- | --- |
| `embedded-plugin-shell --plugin-id growth` | `ok=true` | `/Users/xuxin/.homeai-qa/artifacts/ios-pwa-visual-embedded-plugin-shell-growth-20260618T035158Z.png` |
| `dark-growth-surfaces` | `ok=true` | `/Users/xuxin/.homeai-qa/artifacts/ios-pwa-visual-dark-growth-surfaces-20260618T035208Z.png` |
...
| Embedded plugin UI adapter or learner UI behavior | `tests/growth-frontend-adapter.test.js`, `tests/growth-embedded-layout.test.js`, `tests/growth-routes.test.js`; host action route changes must prove every manifest action (`today_tasks`, `cards`, `submit_work`, `review`, `stage_assessment`, `rewards`) reaches its route-state contract, covers Owner versus learner behavior where different, renders the expected board lane or visible empty/unavailable state, and proves `submit_work`, `review`, and `stage_assessment` do not open unrelated first cards when no structured matching task exists. Embedded iOS APP/PWA layout changes must prove Growth consumes Home AI `hermes.plugin.viewport` top safe-area metrics, exposes `--growth-host-top-safe-area`, reserves `.growth-shell` top scroll space through `--growth-shell-top-reserve`, and does not render page headers or sticky controls under the physical status-bar / Dynamic Island region. Owner Reference Chain UI changes must prove API client proxy/non-proxy reference paths, summary-only panel rendering, visible partial failures, and no browser-side policy reconstruction. Owner stage-checkpoint UI changes must prove formal rubric readback is rendered from bounded service DTO fields, not recomputed in the browser. Owner operating-loop UI changes must prove API client proxy/non-proxy runs/advance paths, summary-only run-history rendering, service-projected run-next payloads, and visible progress/errors without browser-side learning-policy selection. Owner generation action UI changes must prove `data-card-generation-action-panel` appears before profile-feedback and release-workbench panels, includes `刷新状态` / `生成卡片` / `规划下一张` / `发布为卡片`, uses the operating-loop primary generate path, keeps the active `生成` tab inside mobile viewport, and has no horizontal overflow on 402px iOS WebView readback. Owner generation management readability changes must also prove `data-card-generation-secondary-readbacks` exists, default disclosures for `profile`, `automation`, `release`, and `structured-preview` are rendered, the action panel stays before the secondary readbacks, the release workbench is inside the secondary readbacks, and the structured input/audit block is not a permanently expanded page section. Owner profile-feedback UI changes must prove direct/proxy `profile-feedback` readback paths, selected-cycle and auto-select payload construction, readable `画像反馈` panel state for pass/missing/blocked responses, refresh after context, selected-cycle, correction, review, and generation changes, and no browser-side Profile V2, reward, recommendation, Gateway, scheduler, or learner-state mutation. Owner closed-loop action-plan UI changes must prove direct/proxy no-write action-plan readback, `闭环下一步` panel rendering, visible blocked/action/error states, dispatch only to existing Growth-owned functions, and no browser-side policy reconstruction, Gateway calls, scheduler permission, or learner-state mutation. Owner audit-review UI changes must prove direct/proxy list and record helpers, selected-cycle selector payloads, visible review rows/action states, `correction_recorded` correction-id gating, and no browser-side profile-feedback computation, cycle fabrication, Gateway calls, card generation, evaluation, scheduler, or learner-state mutation. Owner proposal-review UI changes must prove blocked controls remain click-observable through `data-automation-proposal-blocked-reason` and do not use browser-side policy to bypass the service. Owner failure-policy UI changes must prove the browser only renders Growth service readbacks and sends bounded create/review commands, never deriving scheduler permission locally. Owner release artifact-template UI changes must prove no-write template/checklist/action-plan rendering, direct/proxy API helper paths, release-workbench-coupled refresh wiring, and no raw artifact path/file upload/visual-tool execution in the browser. Owner release action-audit UI changes must prove direct/proxy action-audits paths, `操作审计` summary-only rendering, refresh wiring, and no browser access to raw request/write-result payloads, release storage, runtime config, or scheduler permission. Owner release status UI changes must prove direct/proxy release controls/dashboard/inventory/review/authorization/closure/preflight/activation/runtime readback paths, `发布总览` summary-only rendering, refresh wiring, and no browser writes to release decisions, package records, preflight reports, activation/runtime rows, runtime config, release permission, or scheduler permission. Owner release evidence ledger UI changes must prove direct/proxy release-evidence and release-approval list paths, `证据账本` summary-only rendering, refresh wiring, and no browser-side evidence writes, approval writes, evidence collection execution, direct release storage access, runtime config mutation, scheduler permission, Gateway call, or learner-state mutation. Owner release lifecycle record UI changes must prove direct/proxy preflight-report, activation-record, and runtime-enablement list/write helpers, `发布记录` summary-only rendering, visible record progress/error/result state, refresh wiring, and no browser-side runtime config mutation, scheduler permission, release approval, evidence collection, Gateway call, or learner-state mutation. | Run Home AI central embedded-plugin visual harness before production UI release. |
