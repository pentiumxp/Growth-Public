...
13. The next planner run uses the updated Profile V2, evidence, audit, and
    summary-only Owner review signal state.
...
| A8: Release-readiness evidence | Growth summarizes product, platform action receipt, visual, stage-checkpoint smoke, production proposal smoke, production cycle-history smoke, production Owner audit smoke, production controlled daily-loop write smoke, production learner-cycle audit smoke, production dry-run, config, reviewed-target, explicit release evidence, release package records, package-record review readback, and bounded remediation fields. | No write permission; advisory release review artifact, package-record status, and next-action guidance only. |
...
| Platform action evidence | `learning-automation-platform-action-evidence-service`; read-only summary evidence over delivered Growth event-outbox action receipts. A passing receipt must contain both an Action Inbox item id and a bounded Web Push summary with `sent > 0`; Growth never reads Home AI Action Inbox/Web Push internals or stores push endpoints. |
| Release-readiness evidence | `learning-automation-release-readiness-service`, `automation-release-readiness.js`, `learning-automation-release-evidence-bundle-audit-service`, `learning-automation-release-collection-run-service`, `automation-release-collection-runs.js`; advisory summary-only release-review checks, remediation plan fields, platform action evidence, central visual evidence, production deployment health evidence, release evidence bundle self-audit evidence, sanitized collection-run records, and snapshots. |
...
| Production deployment evidence | `learning-automation-production-deployment-evidence-service`; read-only validation of Home AI macOS deployment-health summaries into summary-only `productionDeploymentHealthEvidence` without running deploy, restart, launchctl, scheduler, Gateway, or runtime-config mutation inside Growth. |
...
- production worker enablement still requires platform action evidence,
  ...
  self-audit evidence, persisted release collection-run evidence, and explicit
  release approval.
...
| Owner audit review closure | `tests/learning-owner-audit-review-repository.test.js`, `tests/learning-owner-audit-review-service.test.js`, `tests/growth-owner-audit-review-smoke-script.test.js`, release-evidence registry/bundle/readiness/evidence-service harnesses, route/architecture guards, and `npm run smoke:owner-audit-review` prove persisted summary-only Owner review decisions over completed-cycle profile-feedback. The default release-bundle `owner_audit_review` task maps read-only smoke output into `productionOwnerAuditReviewSmokeEvidence`; release-readiness checks it as `production_owner_audit_review_smoke_evidence` and requires a bounded review summary before pass. This row is an audit fact, not learner evidence, generation permission, scheduler permission, or profile mutation. |
| Owner review signal | `tests/learning-owner-review-signal-service.test.js`, `tests/learning-planner-context-service.test.js`, `tests/learning-loop-state-service.test.js`, and `tests/growth-learning-loop-state-smoke-script.test.js` prove summary-only review decisions enter planner context, profile feedback, and loop-state evidence as advisory strategy bias without exposing Owner notes or creating writes. |
