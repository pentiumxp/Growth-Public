...
- proposal selected-cycle create/list/review/accepted-publish UI now has a
  ...
  handoffs, execute actions, schedule workers, or call Gateway. Digest creation
  and worker-target UI remain future work;
...
- central embedded-plugin visual release evidence exists for the Growth plugin
  ...
  digest/action/execution/scheduler-run local UI exists, but
  digest/action/execution/scheduler-run production UI/release evidence plus
  worker-target UI still need their own evidence before those gates can pass;
