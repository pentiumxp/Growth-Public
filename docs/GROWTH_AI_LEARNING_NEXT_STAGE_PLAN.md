...
| UI-gate release evidence | `tests/learning-automation-ui-evidence-service.test.js`, `tests/growth-ui-evidence-smoke-script.test.js`, `tests/growth-architecture-boundary.test.js`, and `npm run smoke:ui-evidence`; validates summary-only artifacts for `ownerDailyUiEvidence`, `ownerAuditUiEvidence`, `proposalReviewUiEvidence`, `automationDigestUiEvidence`, `automationActionHandoffUiEvidence`, `schedulerExecutionUiEvidence`, `schedulerRunUiEvidence`, and `schedulerWorkerTargetUiEvidence`. The service requires matching gate metadata, gate-specific coverage ids, passing assertions, screenshot or DOM evidence, and private-value-safe public projection. It is a validator only and must not run Home AI visual tooling, call Gateway, persist release evidence, publish, generate, evaluate, execute scheduler actions, deliver notifications, activate stage assessments, inspect SQLite directly, or mutate learner state. |
