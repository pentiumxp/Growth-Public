...
- proposal selected-cycle create/list/review/accepted-publish UI now has a
  ...
  call Gateway. Digest creation, scheduler run, and worker-target UI remains
  future work;
...
- central embedded-plugin visual release evidence exists for the Growth plugin
  ...
  digest/action/execution local UI exists, but digest/action/execution
  production UI/release evidence plus scheduler run and worker-target UI still
  need their own evidence before those gates can pass;
