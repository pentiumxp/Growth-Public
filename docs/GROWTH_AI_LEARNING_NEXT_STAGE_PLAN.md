...
- summary-only Owner audit review closure through
  ...
  activating stage assessments. The embedded Owner generation page now
  consumes this boundary through `完成周期审核`: it lists review rows, records
  decisions from selected history-cycle selectors plus bounded Owner note,
  disables `correction_recorded` unless that selected cycle already carries a
  correction id, and refreshes loop/reference/readback state after a write
  without computing profile feedback in the browser;
...
- Owner planner/provision UI now supports visible target selection,
  ...
  current-cycle audit drilldown. It also renders the service-owned
  operating-loop facade as a `闭环执行` panel: the browser reads
  `GET /api/v1/growth/learning-loop/runs`, sends only
  `action=run_next` to Owner-only
  `POST /api/v1/growth/learning-loop/advance`, shows progress/error/run
  history, and never recomputes next-action policy locally. It also renders
  stage-checkpoint controls from
...
4. Owner either clicks `闭环执行` / `执行下一步` to run the current
   service-projected action, clicks `生成卡片` for one explicit
   draft-then-publish action, or drafts a plan first.
...
| --- | --- |
| Planner/author/evaluator Gateway clients | Fake valid stream, valid JSON, empty output, invalid JSON, timeout, repair failure, privacy-risk output, fetch-like HTTP 4xx/5xx Responses object normalization into bounded `gateway_http_error` plus status and gateway error code/type, `tests/growth-planner-readiness-smoke-script.test.js`, `npm run smoke:planner-readiness`, and top-level `plannerReadiness*` smoke operator readback that mirrors the nested planner readiness DTO including bounded Gateway failure fields without adding writes, publication, generation, evaluation, scheduler, notification, stage activation, direct repository access, release-bundle mapping changes, raw response bodies, provider messages, prompts, credentials, or private payloads. |
...
| Learning operating loop | `tests/learning-operating-loop-service.test.js`, `tests/growth-operating-loop-smoke-script.test.js`, route Owner-write tests, `tests/growth-frontend-adapter.test.js`, and `tests/growth-architecture-boundary.test.js`. `learning-operating-loop-service.recommend()` is no-write readback over the current loop state. `runNext()` may execute only the current `nextAction`: `draft_daily_plan` through `learning-daily-loop-service.advance()`, `publish_selected_plan_item` through `learning-daily-loop-service.publish()`, and `review_stage_assessment` through `learning-stage-assessment-service.activateStageAssessment()` only after explicit Owner checkpoint confirmation. `npm run smoke:operating-loop` defaults to no-write recommendation; `run-next` / `advance` require `--allow-write`; formal checkpoint activation also requires `--allow-stage-activation` or `--confirm-stage-assessment`. The Owner browser UI harness must prove API client direct/proxy paths for `GET /learning-loop/runs` and `POST /learning-loop/advance`, summary-only run-history rendering, current-action button payload construction, progress/error visibility, generated card id handoff to the existing preview/open-card flow, and no raw prompt/transcript/private fields in payloads. Unsupported learner, audit/correction, target-provisioning, graph-import, context-refresh, and Gateway-configuration next actions must return visible blocked/separate-flow states, not automatic side effects. |
| Owner audit-review UI closure | `tests/growth-frontend-adapter.test.js`, route Owner-write tests, Owner audit-review repository/service/smoke tests, and architecture guards. The embedded Owner `完成周期审核` panel must prove direct/proxy API client paths for `GET` and `POST /api/v1/growth/owner-audit/reviews`, visible review-history rows, selected-cycle payload construction from cycle-history selectors, bounded Owner note handling, `accepted` / `needs_follow_up` / `correction_recorded` / `blocked` controls, correction-id gating for `correction_recorded`, action progress/error readback, context/loop/reference refresh after record, and no raw prompt/transcript/private fields in payloads. The browser must not compute profile feedback, fabricate cycle selectors, create correction evidence, call Gateway, generate/evaluate cards, schedule, activate stages, or mutate learner state outside the Owner-only review row written by the service. |
...
| Cycle closure review packet | `tests/learning-automation-cycle-closure-service.test.js`, `tests/growth-automation-cycle-closure-smoke-script.test.js`, route/frontend/architecture tests, `npm run smoke:cycle-closure`, `npm run smoke:home-ai-proxy -- --operation cycle-closure`, and top-level `automationCycleClosure*` operator readback prove the one-action bridge from completed cycle to accepted proposal plus pending digest. The service auto-selects a completed cycle only through profile-feedback/cycle-history readback, delegates all writes to proposal/digest/action-handoff services, rejects privacy-risk keys, keeps `publishPerformed=false` and `schedulerStarted=false`, and has architecture guard coverage for no direct Gateway, old Home AI Growth server logic, publication, generation, evaluation, scheduler execution/tick, stage activation, learner-state mutation, direct repository/table access, or Home AI proxy bypass. |
| Review advancement | `tests/learning-automation-review-advancement-service.test.js`, `tests/growth-automation-review-advancement-smoke-script.test.js`, route/frontend/architecture tests, `npm run smoke:review-advancement`, `npm run smoke:home-ai-proxy -- --operation review-advancement`, and top-level `automationReviewAdvancement*` operator readback prove the one-action bridge from completed-cycle review packet to reviewed digest, active failure policy, and action handoff. The service delegates writes only to cycle-closure/digest/failure-policy/action-handoff services, optionally records default-disabled scheduler execution evidence through the scheduler-execution service, rejects privacy-risk keys, keeps handoff delivery and scheduler execution off by default, keeps `publishPerformed=false` and `schedulerStarted=false` by default, and has architecture guard coverage for no direct Gateway, old Home AI Growth server logic, card generation/evaluation, direct publication, scheduler tick, worker timer, stage activation, learner-state mutation, runtime/deploy switch, direct repository/table access, or Home AI proxy bypass. |
...
Release preflight production closure gate note: the nested
`productionClosureGateSummary` / `productionClosureGates` readback classifies
Growth backend readiness, Home AI central visual/UI artifacts, Home AI Action
Inbox/Web Push receipts, Owner release activation, runtime enablement readback,
and production deployment/health evidence. `production_deployment_health`
remains external to Growth local preflight and cannot pass until the Home AI
macOS deployment contract has produced bounded production health evidence and Growth has validated it through `learning-automation-production-deployment-evidence-service` or a matching persisted release-evidence record.
