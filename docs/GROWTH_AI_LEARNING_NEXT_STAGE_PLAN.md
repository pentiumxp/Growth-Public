...
- Owner learning-loop state readback through `learning-loop-state-service`,
  ...
  readiness, next action, target/scope, readiness gates,
...
- release evidence bundle builder CLI:
  ...
  own `--allow-write` gate and bounded top-level `dailyLoop*` operator
  readback. Final controls readback can be collected only by
...
| Stage assessment | Readiness, activation, coverage, completion, cooldown, direct daily-publish blocking, `tests/growth-stage-assessment-smoke-script.test.js`, `npm run smoke:stage-assessment`, and top-level `stageAssessment*` operator readback mirroring nested readiness/activation/completion DTOs; the CLI defaults to read-only readiness and requires explicit `--allow-write` for eligibility, activation, or completion evidence. |
