...
| Daily loop service | Preview, draft, advance, publish, failed publish, audit refresh, ordinary daily duration validation/persistence at 10-15 minutes, `tests/growth-daily-loop-preview-smoke-script.test.js`, `tests/growth-daily-loop-smoke-script.test.js`, `npm run smoke:daily-loop-preview`, controlled `npm run smoke:daily-loop` with explicit `--allow-write` for draft/publish/advance, top-level `dailyLoop*` smoke operator readback from both preview and controlled CLIs that mirrors nested preview/draft/advance/publish DTOs, and no direct Gateway/card-generation calls from routes or the CLI. |
