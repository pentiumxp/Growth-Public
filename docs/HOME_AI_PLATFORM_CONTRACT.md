...
| `plugin_learning_release_evidence_bundle` | `npm run smoke:release-evidence-bundle`; Growth-owned summary-only release evidence bundle builder. It delegates to `learning-automation-release-evidence-bundle-service`, runs selected no-write/default-disabled smoke CLIs through an injected command runner, and emits versioned `growth.learningAutomationReleaseEvidenceBundle.v1` evidence for `npm run smoke:release-readiness -- --evidence-bundle-file <path>`. The default task set includes planner readiness, daily-loop preview, learning-loop state, `cycle_history` from `npm run smoke:cycle-history` mapped to `productionCycleHistorySmokeEvidence`, `owner_audit` from `npm run smoke:owner-audit` mapped to `productionOwnerAuditSmokeEvidence`, `profile_feedback`, audit-only `learner_cycle`, stage-assessment readiness, proposal, platform action evidence from `npm run smoke:platform-action-evidence` mapped to `platformActionEvidence`, central visual evidence from `npm run smoke:central-visual-evidence` mapped to `centralVisualEvidence`, scheduler dry-run, action handoff, scheduler execution/run, worker target, worker smoke, and release-approval bag projection. The explicit `daily_loop_write` task is outside the default set and requires `--allow-write-evidence` plus the daily-loop smoke script's own write gate. After a bundle is written, release tooling should run `npm run smoke:release-evidence-bundle-audit -- --release-evidence-bundle-file <path>` and pass the resulting `releaseEvidenceBundleAudit` into release-readiness; the audit script does not run smoke tasks and is not embedded into the bundle being audited. This boundary must not call Gateway, routes, repositories, daily-loop services directly, cycle-history services directly, Owner audit services directly, learner-cycle services directly, visual tooling, plan publication, card generation, evaluation, scheduler execution/ticks, notifications, stage activation, learner-state mutation, or deploy/release switches. |
