...
function publicScope(input = {}) {
  ...
  const scope = {
    ...
    releasePackageReviewUiEvidence: booleanFlag(input.releasePackageReviewUiEvidence || input.release_package_review_ui_evidence),
...
function taskSpecificArgs(task, scope) {
  ...
  if (task.taskId === "release_controls" || task.taskId === "release_inventory" || task.taskId === "release_dashboard" || task.taskId === "release_workbench") {
    ...
    const evidenceFlags = [
      ...
      ["--release-package-review-ui-evidence", scope.releasePackageReviewUiEvidence],
