(async function bootGrowthPlugin() {
  ...
  function bindEvents() {
    ...
    root.querySelectorAll("[data-release-status-readbacks-refresh]").forEach((button) => {
      button.addEventListener("click", (event) => {
        event.preventDefault();
        if (button.disabled) return;
        refreshReleaseStatusReadbacks().catch((error) => {
          pageState.cardGeneration.releaseStatusReadbacks = Object.assign({}, pageState.cardGeneration.releaseStatusReadbacks, {
            status: "failed",
            error: error.message || String(error)
          });
          renderShell();
  ...
  async function refreshReleaseStatusReadbacks(targetWorkspaceId = cardGenerationWorkspaceId(), context = pageState.cardGeneration.context, options = {}) {
    if (!pageState.auth.isOwner || !context) return null;
    const requestedTargetWorkspaceId = clean(targetWorkspaceId || currentWorkspaceId);
    const previous = pageState.cardGeneration.releaseStatusReadbacks || {};
    pageState.cardGeneration.releaseStatusReadbacks = {
      status: "loading",
      data: previous.data || null,
      error: ""
    ...
    try {
      const ui = window.HermesGrowthCardGenerationUi;
      const payload = ui.createReleaseStatusReadbackQueryPayload({ context, workspaceId: requestedTargetWorkspaceId });
      const result = await api.fetchGrowthReleaseStatusReadbacks(payload, requestedTargetWorkspaceId);
      pageState.cardGeneration.releaseStatusReadbacks = {
        status: "ready",
        data: result,
        error: ""
